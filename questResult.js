...
Question1 = async () => {
    await getResponseData("battles.json").then((battles) => {
        ...
        result.most_active.attacker_king = findMax(attacker_kings)
        result.most_active.defender_king = findMax(defender_kings)
        result.most_active.region = findMax(regions)
        result.most_active.name = findMax(names)
